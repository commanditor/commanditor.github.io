...
class WelcomeModalWidget extends Widget {
	...
	_layout() {
		...
		let w = Math.max(5, Math.min(WelcomeModalWidget.WIDTH, editorLayout.width - 40));
		let h = Math.max(5, Math.min(WelcomeModalWidget.HEIGHT, editorLayout.height - 40));
...
registerThemingParticipant((theme, collector) => {
	...
	if (widgetBackground) {
		collector.addRule(`.monaco-editor .welcomeModalWidget { background-color: ${widgetBackground}; }`);
	}

	const widgetShadowColor = theme.getColor(widgetShadow);
	if (widgetShadowColor) {
		collector.addRule(`.monaco-editor .welcomeModalWidget { box-shadow: 0 2px 8px ${widgetShadowColor}; }`);
	}

	const hcBorder = theme.getColor(contrastBorder);
	if (hcBorder) {
		collector.addRule(`.monaco-editor .welcomeModalWidget { border: 2px solid ${hcBorder}; }`);
	}
});