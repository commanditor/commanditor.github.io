...
export class ConfigController extends Disposable {
    ...
    handleLoggedInChange(b) {
        ...
        this.drive_getOrCreateAppConfig()
            .then(config => {
        ...
        if (this.validateConfigPart(key, value)) {
            switch (key) {
                case 'theme':
                    ...
                    return true;
                default:
                    console.error('tried to update config value without handling updating it');
                    return false;
            }
        } else {
            console.error('config update ignored. supplied key and value would result in invalid config.', key, value);
            return false;
        }
    }
        ...
        return this._drive.getAppConfigFileInfo()
            .then(configFileInfo => {
                if (configFileInfo) {
                    ...
                    return this._drive.createEmptyAppConfigFile()
